...
const addCursorHover = () => {
    ...
    hoverables.forEach(item => {
        item.addEventListener('mouseenter', () => {
            ...
            cursor.style.backgroundColor = 'rgba(0, 245, 212, 0.04)';
        });
...
const renderProfiles = (filteredCandidates) => {
    ...
    filteredCandidates.forEach(candidate => {
        ...
        card.className = 'profile-card cyber-panel';
        card.innerHTML = `
            ...
            <p style="font-size: 13.5px; color: var(--text-secondary); margin-bottom: 12px; flex-grow: 1; font-weight: 300;">${candidate.bio}</p>
    ...
    addCursorHover();
    setupGlareElements(); // Setup glare trackers for newly created panels
};
...
// --- 3D TILT EFFECT & INTERACTIVE GLASS GLARE LOGIC ---
const setupGlareElements = () => {
    const panelsAndCards = document.querySelectorAll('.cyber-panel, .tilt-card');
    
    panelsAndCards.forEach(element => {
        // Track mouse movement to update glare overlay
        element.addEventListener('mousemove', (e) => {
            ...
            // Set dynamic properties for CSS glare positioning
            ...
            if (element.hasAttribute('data-tilt')) {
                ...
                element.style.transform = `rotateX(${tiltX}deg) rotateY(${tiltY}deg) translateZ(10px)`;
                
                // Calculate dynamic glare angle based on cursor position
                const angle = Math.atan2(y - centerY, x - centerX) * (180 / Math.PI);
                element.style.setProperty('--glare-deg', `${angle}deg`);
            }
        });
